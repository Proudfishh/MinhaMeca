...
<body class="bg-surface min-h-screen flex" x-data="{ sidebarOpen: false, maisOpen: false }">
    ...
    @php
        $navItems = \App\Support\Nav::itensPermitidos(auth()->user());
    @endphp
    ...
    @php
        $mobileNav = collect($navItems)->where('mobile', 'primary')->values()->all();
        $maisNav   = collect($navItems)->where('mobile', 'mais')->values()->all();
    @endphp
